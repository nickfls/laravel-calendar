...
class Calendar 
{
  ...
  {
    return  $this->getDayDate($date) == $this->today
            ? $className
            : $default;
  }
  ...
  {
    return  $this->hasEventsOnADate($date)
            ? $className
            : $default;
  }
  ...
  private function getEventsOnADate( $date )
  {
    ...
    $events =  array_first($eventlinks, 
                        function($eventDate, $url) use ($dayDate)
                        {
